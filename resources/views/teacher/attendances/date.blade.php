...
<div class="container-xxl">
    <div class="d-flex align-items-center justify-content-between mb-6">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('teacher.attendances.calendar', $schoolClass->id) }}" class="btn btn-light">Kembali ke Kalender</a>
        </div>
    </div>
                            ...
                            <tr>
                                <td>
                                    <div class="symbol symbol-45px symbol-circle">
                                        <img alt="Pic" src="{{ $student->photo_url ?? asset('assets/media/avatars/blank.png') }}" />
                                    </div>
                                </td>
                                <td>{{ $student->nis ?? '-' }}</td>
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        <button class="btn btn-sm btn-light" data-kt-action="detail" data-student='{{ json_encode(["id"=>$student->id,"nis"=>$student->nis,"name"=>$student->full_name,"parent_name"=>$student->parent_name, "parent_email"=>$student->parent_email, "photo"=>($student->photo_url ?? asset('assets/media/avatars/blank.png'))]) }}'>Detail</button>
                                        <button class="btn btn-sm btn-primary" data-kt-action="absen" data-student-id="{{ $student->id }}" data-current-status="{{ $att->status ?? '' }}" data-remark="{{ $att->remark ?? '' }}">Absen</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <dl class="row mb-0">
            <dt class="col-sm-4">NIS</dt><dd class="col-sm-8" id="dNisDup">-</dd>
            <dt class="col-sm-4">Nama</dt><dd class="col-sm-8" id="dNameDup">-</dd>
            <dt class="col-sm-4">Orang Tua</dt><dd class="col-sm-8" id="dParentDup">-</dd>
            <dt class="col-sm-4">Email Orang Tua</dt><dd class="col-sm-8" id="dParentEmail">-</dd>
        </dl>
      </div>
...
<div class="modal fade" id="absenModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form method="POST" action="{{ route('teacher.attendances.store', [$schoolClass->id, $date]) }}" id="absenForm">
        @csrf
        <div class="modal-header">
