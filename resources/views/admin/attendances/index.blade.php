...
@section('content')
    ...
    <div class="card mb-6">
        <div class="card-body">
            <form method="GET" action="{{ route('attendances.index') }}" class="row g-4 align-items-end">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="recorder_id" class="form-select">
                        <option value="">Semua</option>
                        @foreach($teachers as $t)
                            <option value="{{ $t->id }}" {{ ($filters['recorder_id'] ?? '') == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 d-flex gap-3">
                    <button class="btn btn-primary" type="submit"><i class="bi bi-search me-2"></i>Filter</button>
                    <a href="{{ route('attendances.index') }}" class="btn btn-light">Reset</a>
                </div>
            </form>
        </div>
    </div>
